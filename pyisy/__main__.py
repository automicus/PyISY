...
async def main(url, username, password, tls_ver, events):
    ...
    node_update_subscriber = None
    system_status_subscriber = None

    try:
        if events:
            isy.websocket.start()
            node_update_subscriber = isy.nodes.status_events.subscribe(
                node_update_handler
            )
        ...
        if node_update_subscriber:
            node_update_subscriber.unsubscribe()
        if system_status_subscriber:
            system_status_subscriber.unsubscribe()
        await isy.shutdown()


def node_update_handler(event: NodeChangedEvent) -> None:
